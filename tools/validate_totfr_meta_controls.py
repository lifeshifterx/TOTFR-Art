...
policy=load(SRC/'TOTFR_Required_GitHub_Protection.json');trust=load(SRC/'TOTFR_Agent_Trust_Boundary.json');evidence=load(SRC/'TOTFR_Runtime_Evidence_Policy.json');pub=load(SRC/'TOTFR_Publication_Boundary.json')
if policy:
 ...
 if policy.get('require_status_source_binding') is not True:fail('required status checks must be source-bound')
 ...
 if policy.get('integration_branch')!='~DEFAULT_BRANCH':fail('integration ruleset must target default branch specifically')
if trust:
 ...
 if trust.get('human_file_attestation_is_security_identity') is not False:fail('file attestation cannot become security identity')
...
if evidence:
 if evidence.get('verification_doctrine')!='VERIFY_THEN_TRUST_EXACT_STATE':fail('evidence-policy doctrine mismatch')
 if evidence.get('visual_artifact_ref_must_be_durable') is not True:fail('durable visual evidence control missing')
 if evidence.get('ephemeral_signed_url_allowed') is not False:fail('ephemeral signed evidence must be prohibited')
if pub:
 if pub.get('internal_control_repository_required_visibility')!='PRIVATE':fail('internal control repository must require PRIVATE visibility')
 if pub.get('current_public_history_is_spoiler_contaminated') is not True:fail('known public-history contamination cannot be cleared by declaration')
 if pub.get('policy',{}).get('control_plane_allowed_in_player_safe_public_repository') is not False:fail('control plane must be prohibited from player-safe public distribution')
 if pub.get('policy',{}).get('future_or_dm_metadata_allowed_in_player_safe_public_repository') is not False:fail('future/DM metadata must be prohibited from public distribution')
...
for p in [ROOT/'tools/validate_totfr_runtime_trust.py',ROOT/'tools/test_totfr_runtime_trust.py',ROOT/'tools/validate_totfr_live_protection.py',ROOT/'tools/validate_totfr_meta_controls.py',ROOT/'tools/test_totfr_meta_controls.py',ROOT/'tools/validate_totfr_role_boundary.py',ROOT/'tools/test_totfr_role_boundary.py']:
...
print('END-OF-FILE SENTINEL: TOTFR-META-CONTROL-VALIDATOR-2026-09-04-V3')
