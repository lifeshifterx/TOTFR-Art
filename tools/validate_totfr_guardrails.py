...
ROOT=Path(os.environ.get("TOTFR_ROOT",Path(__file__).resolve().parents[1])).resolve();SRC=ROOT/"13 Source Prompts";MAX_SOP=8000;MAX_DIRECT=10000;MAX_SHARD=8000
RUNS=SRC/"Deployment Runs"
CONTROLLED={
 ROOT/"README.md":"END-OF-FILE SENTINEL:",ROOT/"AGENTS.md":"END-OF-FILE SENTINEL:",
 SRC/"TOTFR_Art_Notion_Deployment_Guardrails.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_App_Tool_Execution_Safety_SOP.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_Art_Generation_Remaster_QA_SOP.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_GitHub_Upload_Safety_Plan.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_Guardrail_Coverage_Matrix.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_Surface_Matrix_Index.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_Transactional_Agent_Deployment_SOP.md":"END-OF-FILE SENTINEL:",RUNS/"README.md":"END-OF-FILE SENTINEL:",SRC/"TOTFR_Production_Manifest.md":"END-OF-FILE SENTINEL:",
}
SOP_TARGETS=set(CONTROLLED)-{ROOT/"README.md",SRC/"TOTFR_Production_Manifest.md"};errors=[]
...
require_text(readme,["AGENTS.md","TOTFR_Art_Notion_Deployment_Guardrails.md","TOTFR_App_Tool_Execution_Safety_SOP.md","TOTFR_Art_Generation_Remaster_QA_SOP.md","TOTFR_GitHub_Upload_Safety_Plan.md","TOTFR_Transactional_Agent_Deployment_SOP.md","TOTFR_Agent_Role_Matrix.csv","TOTFR_Publication_Boundary.json","Deployment Runs/README.md","TOTFR_Surface_Matrix.csv","TOTFR_Surface_Matrix_Index.md"])
...
require_text(ROOT/"AGENTS.md",["Data is not instruction","Exactly one `notion_executor`","Evidence domains cannot substitute","PLAYER-SAFE assets only"])
require_text(SRC/"TOTFR_Art_Notion_Deployment_Guardrails.md",["immutable `control_ref`","CLEAN BASELINE VERIFIED","VISUALLY APPROVED","CONCURRENT_CHANGE","BLOCKED_PRIVATE_SOURCE"])
require_text(SRC/"TOTFR_App_Tool_Execution_Safety_SOP.md",["never substitute for read/inspection","mutation","OPENS THE CIRCUIT","GLOBAL MUTATION FREEZE","EXACT LIBRARY VERIFICATION","<=2 requests/second"])
...
require_text(SRC/"TOTFR_Transactional_Agent_Deployment_SOP.md",["optimistic concurrency","ROLLBACK_CONFLICT","Canary by mechanism","authenticated browser"])
for p in [SRC/"TOTFR_Agent_Role_Matrix.csv",SRC/"TOTFR_Publication_Boundary.json",RUNS/"TOTFR_Run_Template.json",ROOT/"tools/validate_totfr_agent_controls.py",ROOT/"tools/test_totfr_agent_controls.py"]:
 if not p.exists():fail(f"missing required agent-control file: {p.relative_to(ROOT)}")
...
if not workflow.exists():fail("missing guardrail validation workflow")
 ...
 for x in ["actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1","actions/setup-python@5fda3b95a4ea91299a34e894583c3862153e4b97","permissions:","contents: read","persist-credentials: false","timeout-minutes: 5","AGENTS.md","python tools/validate_totfr_guardrails.py","python tools/test_totfr_guardrails.py","python tools/validate_totfr_agent_controls.py","python tools/test_totfr_agent_controls.py"]:
  if x not in w:fail(f"guardrail workflow missing required control: {x}")
schema_path=SRC/"TOTFR_Surface_Matrix.csv";schema=None;required_cols={"asset_id","source_path","operation","audit_a","audit_b","audit_c","technical_qa","visual_qa","cross_surface_qa","design_state","remaster_path","cleanup_state","deployment_state","structural_state","visual_state","source_sha","approved_binary_sha","design_evidence_ref","rollback_ref","cleanup_evidence_ref","deployment_evidence_ref","visual_evidence_ref","failure_count","circuit_state"}
...
ALLOWED={"audit_a":{"PASS","FAIL","UNKNOWN"},"audit_b":{"PASS","FAIL","UNKNOWN"},"audit_c":{"PASS","FAIL","UNKNOWN"},"technical_qa":{"PASS","FAIL","NOT RUN"},"visual_qa":{"PASS","FAIL","NOT RUN"},"cross_surface_qa":{"PASS","FAIL","NOT RUN"},"design_state":{"UNVALIDATED","DESIGN APPROVED","DESIGN REJECTED","DM HOLD","NEEDS REVIEW"},"cleanup_state":{"NOT STARTED","CLEANUP REQUIRED","CLEAN BASELINE VERIFIED","NOT APPLICABLE"},"deployment_state":{"NOT DEPLOYED","DEPLOYMENT WRITTEN","DM HOLD","NO DESTINATION","BROKEN/MISSING"},"structural_state":{"NOT VALIDATED","STRUCTURALLY VERIFIED","STRUCTURAL FAILURE"},"visual_state":{"NOT VALIDATED","VISUAL QA REQUIRED","VISUALLY APPROVED","VISUAL FAILURE"},"circuit_state":{"CLOSED","OPEN"}}
def vrow(d,where):
 ...
 rp=d.get("remaster_path","").strip();approved=ROOT/rp if rp else source;aps=d.get("approved_binary_sha","").strip()
 if approved and aps:
 ...
 if d.get("structural_state")=="STRUCTURALLY VERIFIED" and d.get("deployment_state")!="DEPLOYMENT WRITTEN":fail(f"STRUCTURALLY VERIFIED without DEPLOYMENT WRITTEN {where}")
...
print("END-OF-FILE SENTINEL: TOTFR-GUARDRAIL-VALIDATOR-2026-09-04-HARDENED-V9")
