...
req(RUNREADME,["plan/*.jsonl","attestations/*.json","wal/<target_id>.json","receipts/<target_id>.json","visual/<target_id>.json","No shared append file","mutation_key"])
roles={}
if ROLES.exists():
 try:
...
def clean_str(x):return isinstance(x,str) and not SIGNED.search(x)
def scan_secret(o,where):
 s=json.dumps(o,separators=(",",":"))
 if SIGNED.search(s):fail(f"signed/secret material persisted in {where}")
def check_run(d):
 ...
 if active:
  ...
  for a in sorted((d/"attestations").glob("*.json")) if (d/"attestations").exists() else []:
   ...
   if not x:continue
   scan_secret(x,f"attestation {a.name}");decisions.append(x)
   ...
   if x.get("reviewer_agent") in {r.get("plan_author_agent"),r.get("notion_executor_agent")}:fail(f"non-independent attestation in run {d.name}: {x.get('reviewer_agent')}")
  if tier2:
 ...
 if r.get("status")=="COMPLETE":
  ...
  if fin:
   scan_secret(fin,f"final {d.name}")
  ...
  for tid,o in mut.items():
   wal=d/"wal"/f"{tid}.json";rec=d/"receipts"/f"{tid}.json";vis=d/"visual"/f"{tid}.json"
   w=loadj(wal,"WAL");q=loadj(rec,"receipt");v=loadj(vis,"visual record")
   if w:
   ...
   if v:
    scan_secret(v,f"visual record {tid}")
...
print("END-OF-FILE SENTINEL: TOTFR-AGENT-CONTROL-VALIDATOR-2026-09-04-V2")
